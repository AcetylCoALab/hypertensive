...
RunSVM <- function(Train_set, Train_label, mode, classVar){
  ...
  svmFuncs$rank <- function(object, x, y){
    weight <- t(object$coefs) %*% object$SV
    weight <- setNames(object = weight[1, ], nm = colnames(weight))
    weight <- data.frame(row.names = names(weight),
                         "Overall" = abs(unname(weight)),
                         "var" = names(weight))
    weight <- weight[order(weight$Overall, decreasing = T), ]
    weight
  }
  
  svmProfile <- rfe(x = Train_set, y = as.factor(Train_label[[classVar]]),
                    sizes = c(seq(5, ncol(Train_set))),
                    rfeControl = rfeControl(functions = svmFuncs), 
                    metric = "Accuracy")
  
  data <- as.data.frame(Train_set[, svmProfile$optVariables])
...
CalPredictScore <- function(fit, new_data, type = "lp"){
  ...
  RS <- quiet(switch(
    ...
    "svm.formula" = attr(predict(fit, as.data.frame(new_data), probability = T), "probabilities")[, "1"], # 
